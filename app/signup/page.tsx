...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-100 via-amber-50 to-orange-50 flex items-center justify-center p-6">
      ...
      {/* Floating Elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-20 left-20 w-3 h-3 bg-emerald-400 rounded-full animate-bounce opacity-60" style={{ animationDelay: '0s' }}></div>
        <div className="absolute top-40 right-32 w-2 h-2 bg-blue-500 rounded-full animate-bounce opacity-60" style={{ animationDelay: '1s' }}></div>
        <div className="absolute bottom-32 left-40 w-4 h-4 bg-purple-500 rounded-full animate-bounce opacity-60" style={{ animationDelay: '2s' }}></div>
        <div className="absolute bottom-20 right-20 w-2 h-2 bg-pink-500 rounded-full animate-bounce opacity-60" style={{ animationDelay: '3s' }}></div>
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        {/* Signup Form */}
        <div className="bg-stone-50/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-stone-200">
          <form onSubmit={handleSignup} className="space-y-6">
                    ...
                    {passwordRequirements.map((req, index) => (
                      <div key={index} className="flex items-center space-x-2">
                        <div className={`w-4 h-4 rounded-full flex items-center justify-center ${
                          req.met ? 'bg-emerald-500' : 'bg-stone-300'
                        }`}>
                          {req.met && <Check className="w-3 h-3 text-white" />}
                        </div>
                        <span className={`text-xs ${req.met ? 'text-emerald-600' : 'text-stone-500'}`}>
                          {req.text}
                        </span>
                      </div>
                    ))}
            ...
            {/* Error Message */}
            {error && (
              <div className="p-4 rounded-2xl border-l-4 border-red-500 bg-red-50">
            ...
            <button
              type="submit"
              disabled={loading || password !== confirmPassword || !passwordRequirements.every(req => req.met)}
              className="group w-full py-4 px-6 rounded-2xl font-bold text-lg text-white transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center space-x-3 shadow-lg"
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-stone-50/80 text-stone-500 font-medium">Already have an account?</span>
            </div>
          </div>
